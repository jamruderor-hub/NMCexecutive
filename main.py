...
# ==========================================
# MESIN PENYIMPANAN ADAPTIF (ANTI-RESET ANDROID)
# ==========================================
def get_safe_dir():
    if os.name == 'nt':
        return os.environ.get('APPDATA', os.path.expanduser("~"))
    else:
        # KUNCI ANDROID: expanduser("~") sering diblokir oleh Scoped Storage.
        # Kita paksa menggunakan os.environ['HOME'] agar masuk ke memori internal absolut.
        home_dir = os.environ.get('HOME')
        if home_dir and os.access(home_dir, os.W_OK):
            return home_dir
        return os.path.expanduser("~")

FILE_CONFIG = os.path.join(get_safe_dir(), "nmc_mobile_config.json")
...
def muat_pengaturan():
    ...
    return {"host": "", "port": "3306", "user": "root", "password": "", "cf_url": "", "refresh_interval": "3", "overtime_limit": "2.0"}
...
def main(page: ft.Page):
    ...
    # --- FUNGSI CLIENT_STORAGE YANG ERROR SUDAH DIHAPUS TOTAL ---
    # Aplikasi sekarang akan memanggil mesin penyimpanan aman 
    # yang ada di bagian paling atas skrip.
            ...
            if GLOBAL_DATA["db_status"] == "Konek DB":
                ...
                if GLOBAL_DATA["produksi"]:
                    for i, (nama, kg) in enumerate(GLOBAL_DATA["produksi"].items()):
                        ...
                        ACTIVE_UI["list_produksi"].controls.append(ft.Row([ft.Row([ft.Container(width=10, height=10, border_radius=5, bgcolor=w), ft.Text(nama, color="#BDC3C7")], spacing=5), ft.Text(f"{kg:,.0f} Kg".replace(",", "."), color="white", weight="bold")], alignment="spaceBetween"))
                else:
                ...
                if data_render:
                    for row in data_render:
                        if sub == "tiket":
                            ...
                            warna_tipe = "#9B59B6" if "Pabrik" in str(row[2]) or "Umum" in str(row[2]) else "#E67E22"
                            ACTIVE_UI["list_transaksi"].controls.append(
                                ft.Container(bgcolor="#141a29", padding=15, border_radius=10, margin=10,
                                    content=ft.Row([
                            ...
                            ACTIVE_UI["list_transaksi"].controls.append(
                                ft.Container(bgcolor="#141a29", padding=15, border_radius=10, margin=10,
                                    content=ft.Column([
                                        ft.Row([ft.Text(f"📦 {row[1]}", weight="bold", color="white", size=12), ft.Text(f"Rp {float(row[2]):,.0f}".replace(",", "."), color="#2ECC71", size=14, weight="bold")], alignment="spaceBetween"),
                                        ft.Text(f"Supplier: {row[0]}", color="#BDC3C7", size=11)
                                    ])
                                )
                            )
                else:
    ...
    def tarik_data_database():
        with DB_LOCK:
            ...
            try:
                ...
                kas_keluar = float(cursor.fetchone()[0] or 0)
                
                cursor.execute("SELECT SUM(total_harga) FROM transaksi WHERE mode='Pembelian' AND status_bayar='Lunas'")
                beli_tbs = float(cursor.fetchone()[0] or 0)

                GLOBAL_DATA["kas_keluar"] = kas_keluar + beli_tbs
                GLOBAL_DATA["sisa_saldo"] = GLOBAL_DATA["kas_masuk"] - GLOBAL_DATA["kas_keluar"]
                ...
                cursor.execute("SELECT jenis_barang, SUM(total_netto) FROM transaksi GROUP BY jenis_barang")
                ...
                cursor.execute("SELECT no_do, SUM(total_netto) as tot FROM transaksi WHERE mode='Pembelian' GROUP BY no_do ORDER BY tot DESC LIMIT 5")
                ...
                cursor.execute("SELECT kode, nama, tipe FROM master_supplier WHERE status='Aktif' AND tipe IN ('Umum', 'Petani Mandiri', 'Pengepul / RAM', 'Koperasi / Plasma') AND nama != 'Pelanggan Umum' LIMIT 100")
                GLOBAL_DATA["mst_suplier"] = cursor.fetchall()
                
                cursor.execute("SELECT kode, nama, tipe FROM master_supplier WHERE status='Aktif' AND (tipe='Pabrik Pembeli PKS' OR nama='Pelanggan Umum') LIMIT 100")
                GLOBAL_DATA["mst_customer"] = cursor.fetchall()
                
                cursor.execute("SELECT nama_supplier, komoditas, nominal FROM master_harga WHERE status='Aktif' ORDER BY tanggal DESC LIMIT 50")
    ...
    def pindah_menu(nama, sub=""):
        ...
        if halaman_aktif == "dashboard":
            ...
            header = ft.Container(bgcolor="#141a29", padding=20, content=ft.Row([ft.Row([ft.Image(src="pht.png", width=30, height=30, fit="contain"), ft.Text("NMC", weight="bold", size=20, color="#3498DB"), ft.Text("Executive", size=20, color="white")], alignment="start", spacing=8), ind_warna], alignment="spaceBetween"))
            ...
            card_top_5 = ft.Container(bgcolor="#141A29", padding=20, border_radius=12, margin=15, content=ft.Column([ft.Text("🏆 TOP 5 CUST & SUPPLIER", weight="bold", color="#E67E22"), ft.Divider(color="#2C3E50"), list_top5]))

            area_tengah = ft.Column([header, card_kasir, card_bahan_baku, card_produksi, card_top_5, ft.Container(height=10)], scroll="auto", expand=True)
        ...
        # ==========================================
            ...
            header_transaksi = ft.Container(bgcolor="#141A29", padding=20, content=ft.Column([ft.Row([ft.Row([ft.Image(src="pht.png", width=25, height=25, fit="contain"), ft.Text("Pusat Data", weight="bold", size=18, color="white")], spacing=10), ind_warna], alignment="spaceBetween")]))
            ...
            menu_tab = ft.Container(padding=15, content=ft.Column([
                ...
                ft.Row([buat_tab("🤝", "Suplier", "suplier"), buat_tab("🏷️", "Harga", "harga")], spacing=10), 
            ], spacing=10))
            ...
            filter_area = ft.Container(
                ...
                content=ft.Column([
                    row_tanggal,
                    ft.Row([inp_cari_nama, btn_terapkan], spacing=10)
                ], spacing=10),
                visible=(sub in ["tiket", "pembayaran", "lap_beli", "lap_kirim"])
            )
        ...
        # ==========================================
            ...
            header_antrean = ft.Container(bgcolor="#141A29", padding=20, content=ft.Column([ft.Row([ft.Row([ft.Image(src="pht.png", width=25, height=25, fit="contain"), ft.Text("Live Yard", weight="bold", size=18, color="white")], spacing=10), ind_warna], alignment="spaceBetween"), ft.Text("Pantau arus kendaraan di dalam pabrik.", color="#BDC3C7", size=12)]))
                ...
                def eksekusi_kirim():
                    ...
                    # ----------------------------------------------------
                        ...
                        try:
                            ...
                            cursor = conn.cursor()
                            # Nama pengirim dipatenkan sebagai 'NMC Executive'
                            cursor.execute("INSERT INTO tabel_chat (waktu, pengirim, pesan) VALUES (%s, 'NMC Executive', %s)", (waktu_skrg, pesan))
                            conn.commit()
                            conn.close()
                        except: pass
        ...
        # ==========================================
            ...
            if sub_halaman_aktif == "":
                header_setting = ft.Container(
                    ...
                    content=ft.Column([
                        ft.Row([
                            ft.Image(src="pht.png", width=35, height=35, fit="contain"), 
                ...
                def aksi_tombol_koneksi(e):
                    if GLOBAL_DATA["db_status"] == "Konek DB":
                        ...
                        lbl_status_simpan.value = "⚠️ Koneksi Berhasil Diputuskan."
                ...
                konten_about = ft.Container(padding=30, content=ft.Column([ft.Container(height=20), ft.Image(src="pht.png", width=100, height=100, fit="contain"), ft.Container(height=10), ft.Text("NMC EXECUTIVE", weight="bold", size=24, color="white"), ft.Text("Versi 1.0.0 (Mobile Edition)", color="#BDC3C7", size=12), ft.Divider(height=40, color="#2C3E50"), ft.Text("Aplikasi ini dibuat khusus untuk memantau operasional jembatan timbang secara real-time.", color="white", text_align="center", size=13), ft.Container(height=40), ft.Text("© 2026 PT. Naila Multimedia Center", color="#3498DB", size=12, weight="bold")], horizontal_alignment="center"))
