...
GLOBAL_DATA = {
    ...
    "bahan_baku": {}, "produksi": {}, "top_5": [], "tren_7_hari": [], 
...
def main(page: ft.Page):
    ...
    # KEMBALI KE DROPDOWN STANDAR YANG AMAN DARI BUG
    f_hrg_sup = ft.Dropdown(label="Nama Supplier", bgcolor="#0A1128", border_color="#3498DB", color="white", text_size=12)
    ...
    def buka_form_master(tabel, aksi, data_baris=None):
        ...
        if tabel == "supplier":
            ...
            # =======================================================
            # SOLUSI PRAGMATIS: MUAT SEMUA DATA SEKALIGUS (BEBAS BUG)
            # =======================================================
            semua_relasi = GLOBAL_DATA.get("mst_suplier", []) + GLOBAL_DATA.get("mst_customer", [])
            opsi_sup = [ft.dropdown.Option(key="- Berlaku Semua -", text="- Berlaku Semua -")]
            for row in semua_relasi:
                nama = str(row[1]).strip()
                tipe = str(row[2]).strip()
                # Teks yang tampil di layar ada keterangan kategorinya, tapi data yang tersimpan tetap "Nama Asli"-nya
                opsi_sup.append(ft.dropdown.Option(key=nama, text=f"{nama} ({tipe})"))
            f_hrg_sup.options = opsi_sup
            # =======================================================
            ...
            if aksi == "tambah":
                f_hrg_kode.value, f_hrg_tgl.value, f_hrg_komoditas.value = "[Otomatis]", datetime.now().strftime("%Y-%m-%d"), "Semua Barang"
                f_hrg_tipe.value = "Umum"
                f_hrg_sup.value = "- Berlaku Semua -"
                f_hrg_nom.value, f_hrg_status.value = "0", "Aktif"
                ...
                nama_sup_lama = data_baris[4]
                if nama_sup_lama in [opt.key for opt in f_hrg_sup.options]:
                    f_hrg_sup.value = nama_sup_lama
                else:
                    f_hrg_sup.value = "- Berlaku Semua -"
    ...
    def update_live_ui():
        ...
        elif halaman_aktif == "transaksi" and "list_transaksi" in ACTIVE_UI:
        ...
        elif halaman_aktif == "antrean" and "list_antrean" in ACTIVE_UI:
        ...
        elif halaman_aktif == "chat" and "list_chat" in ACTIVE_UI:
            ACTIVE_UI["list_chat"].controls.clear()
            if GLOBAL_DATA["db_status"] == "Konek DB":
                if GLOBAL_DATA["chat_history"]:
                    for r in reversed(GLOBAL_DATA["chat_history"]): 
        ...
        elif halaman_aktif == "pengaturan" and sub_halaman_aktif == "server" and "btn_koneksi" in ACTIVE_UI:
            if GLOBAL_DATA["db_status"] == "Konek DB":
        ...
        with DB_LOCK:
            ...
            if cf_url != "":
                try:
                    ...
                    GLOBAL_DATA["tren_7_hari"] = data_api.get("tren_7_hari", []) 
